...
class MyStrategy(bt.Strategy):
    ...
    def rebalance_portfolio(self):
        ...
        long_list=[]
        sorted_rate=sorted(rate_list,key=lambda x:x[1])
        # for stock in sorted_rate[:int(len(sorted_rate)/10)]:
        #     print(stock[0], stock[1][0])
        if np.isnan(sorted_rate[0][1][0]):
            return
        long_list=[i[0] for i in sorted_rate[:int(len(sorted_rate)/10)]]
        ...
        p_value = total_value*0.9/3
        for data in self.datas:
            #获取仓位
            pos = self.getposition(data).size
            if not pos and data._name in long_list:
                size=int(p_value/100/data.close[0])*100
                o1 = self.buy(data = data, size = size) 
                # o2 = self.buy(data = data, size = size*0.9, oco=o1)
                # o3 = self.buy(data = data, size = size*0.85, oco=o1)
                # o3 = self.buy(data = data, size = size*0.8, oco=o1)
...
def backTest(name):
    ...
    if os.path.exists("Result.csv"):
        with open("Result.csv", "w") as reference:
            temp_df = pd.read_csv("Result.csv")
            temp_df[f"{name} Return"] = df[f"{name} Return"]
            temp_df.to_csv(reference, sep = ",", index = False)
    else:
